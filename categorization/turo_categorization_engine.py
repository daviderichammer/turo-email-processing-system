#!/usr/bin/python3
"""
Turo Email Dynamic Categorization and Duplicate Detection Engine
Intelligently categorizes emails and detects duplicates with learning capabilities
"""
...
class TuroCategorizer:
    ...
    def generate_content_hash(self, email_data: Dict) -> str:
        """Generate a hash for email content to detect exact duplicates"""
        ...
        if email_data.get('subject'):
            # Normalize subject by removing encoded parts and extra whitespace
        ...
        if email_data.get('body_text'):
            # Normalize body by removing URLs, extra whitespace, and common variations
            body = email_data['body_text']
            body = re.sub(r'https?://[^\s]+', '[URL]', body)
            body = re.sub(r'\s+', ' ', body.lower().strip())
            content_parts.append(body[:500])  # First 500 chars for efficiency
        
        content_string = '|'.join(content_parts)
        return hashlib.md5(content_string.encode('utf-8')).hexdigest()
    
    def detect_duplicates(self, email_id: int, email_data: Dict) -> List[Dict]:
        """Detect duplicate emails using multiple methods"""
        duplicates = []
        
        try:
            cursor = self.db_connection.cursor(dictionary=True)
            
            # Method 1: Exact content hash matching
            content_hash = self.generate_content_hash(email_data)
            
            # Check for emails with similar content
            query = """
                SELECT id, sender_email, subject, body_text, received_date
                FROM emails 
                WHERE id != %s 
                AND sender_email = %s
                AND received_date >= DATE_SUB(NOW(), INTERVAL 30 DAY)
            ...
            for similar_email in similar_emails:
                ...
                # Subject similarity
                subject_similarity = self.calculate_text_similarity(
                    email_data.get('subject', ''), 
                    similar_email.get('subject', '')
                )
                
                # Body similarity (first 1000 chars for efficiency)
                body_similarity = self.calculate_text_similarity(
                    email_data.get('body_text', '')[:1000],
                    similar_email.get('body_text', '')[:1000]
                )
                
                # Combined similarity score
                combined_similarity = (subject_similarity * 0.4) + (body_similarity * 0.6)
                
                if combined_similarity >= 0.90:
                    duplicates.append({
                        'email_id': similar_email['id'],
    ...
    def process_email(self, email_id: int) -> Dict:
        ...
        try:
            ...
            logging.info(f"Processing email {email_id}: duplicate detection")
            ...
            if duplicates:
                ...
                if best_duplicate['similarity_score'] >= 0.90:
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Turo Email Categorization Engine")
